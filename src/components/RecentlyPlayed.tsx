...
function RecentlyPlayed() {
  const {
    ...
    play,
    pause,
    currentlyPlaying: { infos: cpInfos },
    isPlaying
  } = useMusic();
  return (
    <IconContext.Provider value={{ size: "28" }}>
      <Grid className="mt-8 text-white">
        <h1 className="col-span-6 text-3xl font-medium">Recently played</h1>
        <div className="col-span-6 flex flex-col gap-y-5 p-2">
          {recentlyPlayed.map(({ author, cover, songName, songPath }) => {
            const isThisSongPlaying = cpInfos?.songPath === songPath
            return (
